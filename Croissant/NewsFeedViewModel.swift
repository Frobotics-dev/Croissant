...
struct RSSFeedConstants {
    ...
    static let mixedModeDisplayName = "All Feeds" // Clear display name for the UI filter
...
class NewsFeedViewModel: NSObject, ObservableObject {
    ...
    var availableFeedsForMenu: [(key: String, value: String)] {
        ...
        combinedFeeds.append((key: "Custom URL", value: "custom"))
    ...
    @Published var feedURLString: String {
        didSet {
            // Save the selected feed preference. This ensures the picker selection is remembered.
            UserDefaults.standard.set(feedURLString, forKey: "newsFeedURLString")
            setupFetchDebounce()
        }
    }
    
    @Published var customFeedURLString: String {
        didSet {
            UserDefaults.standard.set(customFeedURLString, forKey: "customFeedURLString")
            // If the user is editing the custom URL, debounce and refetch.
            if feedURLString == "custom" {
                setupFetchDebounce()
            }
        }
    ...
    var feedURL: URL? {
        if feedURLString == "custom" {
            return URL(string: customFeedURLString)
        }
    ...
    override init() {
        ...
        // Load the saved custom URL
        let initialCustomURL = UserDefaults.standard.string(forKey: "customFeedURLString") ?? ""
        _customFeedURLString = Published(initialValue: initialCustomURL)
    ...
    private func setupFetchDebounce() {
        ...
        // Don't fetch if custom is selected and the URL is empty.
        let isCustomAndEmpty = feedURLString == "custom" && customFeedURLString.isEmpty
        guard !isCustomAndEmpty else {
            // Clear items if custom URL is cleared
            DispatchQueue.main.async {
                self.newsItems = []
        ...
        // Only load news if a valid selection is made
    ...
    private func fetchSingleNews() async {
        guard let feedURL = self.feedURL else {
            ...
            await MainActor.run {
                // Determine which URL string is relevant for the check
                let relevantURLString = (feedURLString == "custom") ? customFeedURLString : feedURLString
                
                if !relevantURLString.isEmpty {
        ...
        do {
            ...
            let items = await Task.detached { [weak self] in
                // Use the concurrency-safe parsing function
                return await self?.parseRSS(data: data, sourceURL: feedURL) ?? []
    ...
    private func fetchAndParseFeed(url: URL) async -> [NewsItem] {
        do {
            ...
            let items = await Task.detached {
                await self.parseRSS(data: data, sourceURL: url)
            }.value
            return items
        } catch is CancellationError {
            return []
